  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      let rec mem_in_env = fun e m -> match e with
      | [] -> ()
      | hd :: tl -> 
        let (env_val, env_loc) = hd in
        match env_loc with
        | Proc p -> mem_in_env tl m
        | Loc (base, offset) ->
          let rec loc_find base_e m = match m with
          | [] -> []
          | ((b, o), v) :: tl_m ->
            if b = base_e then (b, o) :: loc_find base_e tl_m else loc_find base_e tl_m
          in
          let locs_same_base = loc_find base m in
          let rec loc_from_locs locs m = match locs with
          | [] -> ()
          | l :: tl_l ->
            let _ =
            if List.mem_assoc l m then 
            match List.assoc l m with
            | R r -> 
              let rec rec_in_mem record = match record with
              | [] -> ()
              | hd_r :: tl_r -> reachable_locs := (snd hd_r) :: !reachable_locs; rec_in_mem tl_r
              in
              reachable_locs := l :: !reachable_locs; rec_in_mem r
            | Z n -> reachable_locs := l :: !reachable_locs
            | L l_m -> reachable_locs := l :: !reachable_locs
            | B b -> reachable_locs := l :: !reachable_locs
            | _ -> ()
            else ()
            in
            loc_from_locs tl_l m
          in
          let _ = loc_from_locs locs_same_base m in
          mem_in_env tl m
      in
      let rec r_find m e k = match k with
        | [] -> mem_in_env e m
        | (next_c, next_e) :: tl_k -> 
        let _ = mem_in_env e m in
         r_find m next_e tl_k
      in
      let _ = r_find m e k in
      (*let _ = print_string (String.concat " " (List.map loc_to_str !reachable_locs)) in*)
