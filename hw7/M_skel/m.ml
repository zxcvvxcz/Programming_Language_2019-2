...
struct
  ...
  let op2fn =
    ...
    | EQ -> fun (v1, v2) -> (* TODO : implement this *)
      match (v1, v2) with
      | (Int i1, Int i2) -> Bool (i1 = i2)
      | (String s1, String s2) -> Bool (s1 = s2)
      | (Bool b1, Bool b2) -> Bool (b1 = b2)
      | (Loc l1, Loc l2) -> Bool (l1 = l2)
      | _ -> raise (TypeError "cannot compare")
  ...
  let rec eval env mem exp = 
    ...
    | LET (dec, e) ->
     ( match dec with
      | REC (id1, id2, e_rec) ->
        let c = Closure (RecFun (id1, id2, e_rec), env) in
        let env' = env @+ (id1, c) in
        eval env' mem e
      | VAL (x, e_val) ->
        let (e_result, m') = eval env mem e_val in
        let env' = env @+ (x, e_result) in
        eval env' m' e)
    | MALLOC e ->
      let (v, m') = eval env mem e in
      let (l, m'') = malloc m' in
